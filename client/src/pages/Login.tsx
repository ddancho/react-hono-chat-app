...
function LoginPage() {
  ...
  const { setAuthUser, setSocketClient } = useAuthContext();
  const { setOnlineUserIds } = useChatContext();
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (signInData: UserSignIn) => {
    const res: ServerResponse = await loginUser(signInData);
    if (res.status === "error") {
    ...
    const socket = connectToSocketServer(res.user.id);
